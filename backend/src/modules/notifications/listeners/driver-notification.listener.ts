...
@Injectable()
export class DriverNotificationListener {
  constructor(
    private readonly notificationsService: NotificationsService,
    private readonly prisma: PrismaService,
  ) {}

  @OnEvent(NOTIFICATION_EVENTS.DRIVER_SUSPENDED, { async: true })
  async handleDriverSuspended(event: NotificationEvent) {
    await this.createNotifications(event);
  }

  @OnEvent(NOTIFICATION_EVENTS.DRIVER_INACTIVE, { async: true })
  async handleDriverInactive(event: NotificationEvent) {
    await this.createNotifications(event);
  }
  ...
  private async resolveRecipients(ids: string[]) {
    if (ids.length === 0) {
      const users = await this.prisma.user.findMany({
        where: { isActive: true, deletedAt: null, role: { name: { in: ['admin'] } } },
        select: { id: true },
      });
