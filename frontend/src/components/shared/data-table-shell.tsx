...
const TABLE_CELL_CLASS = 'h-10 px-4 py-2 align-middle text-center whitespace-nowrap text-sm';
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full caption-bottom text-base">
            <thead className="sticky top-0 z-20 border-b border-border bg-card/90 backdrop-blur-sm dark:border-border/30 dark:bg-card/90">
                ...
                <tr key={headerGroup.id} className="border-b border-border/40 transition-colors dark:border-border/30">
                  {headerGroup.headers.map((header) => (
                    <th
                      key={header.id}
                      className={cn(
                        'h-10 px-4 align-middle text-center text-xs font-semibold whitespace-nowrap text-muted-foreground tracking-wide uppercase',
                        header.column.getCanSort() && 'cursor-pointer select-none transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/30',
                    ...
                    >
                      <div className="flex items-center justify-center gap-1.5">
                        ...
                        {header.column.getCanSort() ? (
                          <span className="inline-flex size-4 items-center justify-center transition-transform duration-200">
            ...
            <tbody>
              {isLoading
                ? Array.from({ length: 5 }).map((_, rowIndex) => (
                    <tr key={rowIndex} className="h-10 border-b border-border/40">
              ...
              {!isLoading
                ? table.getRowModel().rows.map((row, rowIndex) => (
                    <motion.tr
                      ...
                      className="h-10 border-b border-border/40 dark:border-border/30"
                    >
