...
export function CreateVehiclePage() {
  ...
  const canCreate = hasPermission(PERMISSIONS.CREATE_VEHICLE);

  function handleSubmit(payload: CreateVehiclePayload) {
    mutation.mutate(payload, {
      onSuccess: (vehicle) => {
        toast({
          title: t('vehicles.vehicle_created'),
          description: t('vehicles.vehicle_created_desc', { code: vehicle.vehicleCode }),
          variant: 'success',
        });
  ...
  if (!canCreate) {
    return (
      <PageSection variant="wrapper">
        <AppBreadcrumbs />
        <PageHeader
          title={t('vehicles.create_vehicle')}
  ...
  return (
    <PageSection variant="wrapper">
      ...
      <PageHeader
        title={t('vehicles.create_vehicle')}
        description={t('vehicles.new_vehicle')}
      />
